...
public class koboldspearthrowmono : animredirect
{
    ...
    void Start()
    {
		...
		if(comb.isPC){
			...
			AItarg = temp[rando].transform.position;
		}
	}
	...
	public override void directed(float f){
		if(!thrown){
			...
			if(comb.isPC){clone3.GetComponent<projectile>().bz.Points = new Vector3[]{startpoint,Vector3.LerpUnclamped(transform.position,trg,power),Vector3.LerpUnclamped(transform.position,trg,power) + new Vector3(Mathf.Abs(transform.position.x-Vector3.LerpUnclamped(transform.position,trg,power).x)*(anim.transform.parent.parent.GetComponent<Combatant>().isPC?2:-2),0,0),endpoint};
			} else {clone3.GetComponent<projectile>().bz.Points = new Vector3[]{hand.position, Vector3.Lerp(hand.position,AItarg,0.5f)+ new Vector3(1,5,0), Vector3.Lerp(hand.position,AItarg,0.5f)+ new Vector3(-1,5,0),AItarg};
			}
